...
class StrippingExample_Shader : IPreprocessShaders
{
    ...
    // Strip entire shader
    public bool StripEntireShader(Shader shader)
    {
        if (
    ...
    // Strip entire pass
    public bool StripEntirePass(ShaderSnippetData snippet)
    {
        if (
            snippet.passName == "EXAMPLE/CoC Temporal Filter"
    ...
    // public bool StripVariant(ShaderCompilerData data)
    // {
    //     if (
    ...
    public bool StripVariant(string variantText)
    {
        if (
            variantText.Contains("_xxxGOKEY2") ||
            variantText.Contains("_xxxLOKEY1")
            )
            return true;
        return false;
    }

    public bool StripLogic(Shader shader, ShaderSnippetData snippet, string variantText)
    {
        if (StripEntireShader(shader))
            return true;

        if (StripEntirePass(snippet))
            return true;

        if (shader.name.IndexOf("xxxGraphicsGeneral_FTP/Shader_Keyword_Global_MultiCompile") >= 0)
            if (StripVariant(variantText))
                return true;
    ...
    public void OnProcessShader(Shader shader, ShaderSnippetData snippet, IList<ShaderCompilerData> data)
    {
        ...
        for (int i = 0; i < data.Count; ++i)
        {
            ...
            if ( StripLogic(shader, snippet, variantText) )
            {
                Debug.Log(CText("Stripped variant: "+shader.name + "(" + snippet.shaderType.ToString() + ") Variant:" + variantText ));
